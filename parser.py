...
class VFSParser:
    ...
    def _parse_folder(self, folder_element: ET.Element) -> Dict[str, Any]:
        """Рекурсивно парсит папку и её содержимое."""
    ...
    def vfs_ls(self) -> str:
        """Возвращает строковое представление содержимого корневой директории VFS (глубина 1)."""
    ...
    def vfs_cd(self, path: str) -> bool:
        """Изменяет текущую директорию на указанный путь.
        Поддерживает как абсолютные (/root/documents), так и относительные (documents/../projects) пути.
    ...
    def vfs_info(self) -> str:
        """Возвращает информацию о загруженной VFS: имя и хеш SHA-256."""
        if not self.vfs_data:
    ...
    def vfs_save(self, save_path: str) -> bool:
        """Сохраняет текущее состояние VFS в XML-файл."""
        try:
    ...
    def _get_current_node(self) -> Dict[str, Any]:
        """Возвращает узел дерева, соответствующий текущей директории."""
        current_node = self.vfs_data
    ...
    def _build_folder_element(self, folder_data: Dict[str, Any]) -> ET.Element:
        """Рекурсивно строит XML-элемент папки."""
